...
function TrianglesLogoSVG({
  className = "",
  style = {},
  leftRightOpacity = "0.08", // opacidade dos triângulos laterais padrão
  centerOpacity = "0.13",    // opacidade do triângulo central (azul) padrão
}: {
  className?: string;
...
export default function Services() {
  ...
  return (
    <section
      ...
      role="region"
    >
      {/* Triângulos/logo espalhados como enfeite de fundo - REMOVIDOS BLOBs */}
      <TrianglesLogoSVG
        className="absolute top-8 left-8 w-28 z-0 pointer-events-none select-none"
        style={{ transform: "rotate(13deg)" }}
      />
      <TrianglesLogoSVG
        className="absolute top-[46%] right-12 w-32 z-0 pointer-events-none select-none"
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {servicesData.map((svc) => (
            ...
            >
              ...
              <p className="text-base text-[#444] mb-4 font-medium italic group-hover:text-primary/80 transition-colors duration-300">
                {svc.subtitle}
              </p>
              <ul className="space-y-2 mt-2">
                {svc.points.map((point) => (
                  <li
                    key={point}
                    className="flex items-center gap-2 text-[#222] transition-colors duration-300"
                  >
        ...
        <div className="flex flex-col items-center my-16">
          <h3 className="text-2xl md:text-3xl font-bold text-primary mb-2 text-center">
            Browse our wide range of color options
          </h3>
